...
# ==========================================
# 方式1: 直接填写（当前使用）
API_KEY = "sk-kimi-6AoSrvT7sujYhKyo4eq1CdhIISNA4Mvx9MPQHsIBGsfMnhpprdAQmTetuyR2Xv25"

# 方式2: 环境变量（如需使用，注释掉上面一行，取消下面注释）
# API_KEY = os.getenv('KIMI_API_KEY', '')

if not API_KEY or '你的完整key' in API_KEY:
    print("错误: 请先配置 API_KEY")
    print("在 https://kimi.com/code 生成 API Key")
    sys.exit(1)
# ==========================================
...
# 提示词（价格行为分析）
PROMPT = """你是专业价格行为分析师。请从图表截图中提取结构化信息，并严格输出 JSON。
...
- pattern_name: Nested Expanding Triangle|Expanding Triangle|Wedge Top|Wedge Bottom|Truncated Wedge Bottom|Bull Measuring Gap|Exhaustion Gap|Small Pullback Bull Trend|Bull Trend|Double Bottom|Double Top|null
...
输出 JSON schema（严格遵守字段名）：
{
  ...
  "kline_features": [{"feature": "double_bottom|double_top|engulfing|inside_bar|outside_bar|gap|doji|null", "confidence": 0.0}],
...
class BatchProcessor:
    ...
    def save_to_postgres(self, page_num: int, image_path: str, data: Dict):
        ...
        try:
            ...
            if existing:
                ...
                ''', (
                    json.dumps(data),
                    data.get('patterns', [{}])[0].get('pattern_name') if data.get('patterns') else None,
                    data.get('slide_type'),
                    pattern_id
                ))
            else:
    ...
    def save_vector(self, pattern_id: int, image_path: str, page_num: int, vector_data: Dict):
        """保存向量到数据库"""
        cursor = self.conn.cursor()
