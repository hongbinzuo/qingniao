...
**生成时间**: 2026-01-10 20:23:42

---


## 文档 1: ABU_ML_DL完整执行方案

**路径**: `docs\design\features\ABU_ML_DL完整执行方案.md`
...
【阶段0: Gemini Vision API完整分析（必须先完成）】
┌─────────────────────────────────────────────────────────────────────┐
│ 0.1 图片准备 (已有)                                                  │
│   └─ data/abu/images/*.png (1000张图片)                             │
│   └─ pattern_library表 (1004条记录，但gemini_annotation_json为空)   │
└─────────────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────────────┐
│ 0.2 Gemini Vision API分析 (abu_gemini_annotate_enhanced.py)         │
│   ├─ 增强Prompt: 提取多模式组合、K线行为、交易参数                   │
│   ├─ 批量分析: 1000张图片                                            │
│   │   ├─ 模型: gemini-1.5-flash (推荐，成本低)                     │
│   │   ├─ 成本: 约$0.125 (Flash) 或 $2.5 (Pro)                      │
│   │   └─ 耗时: 约20-30分钟 (分批处理)                               │
...
┌─────────────────────────────────────────────────────────────────────┐
│ 1.1 模式库准备 (增强后)                                              │
│   └─ pattern_library (1004条，已包含Gemini分析)                     │
...
└─────────────────────────────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────────────────────────────┐
│ 1.2 信号生成流程 (增强版: pa_scan_15m_top10.py)                      │
│   ├─ 扫描TOP30币种 (BTC, ETH, SOL等)                                │
│   ├─ 获取K线数据 (15m + 1h)                                          │
│   ├─ 模式检测 (detect_all_15m - 增强版)                              │
│   │   ├─ 传统检测 (detect_inside_bar, engulfing, pin_bar等)        │
│   │   ├─ Gemini模式匹配 (新增)                                       │
...
│   │       ├─ 验证是否匹配Gemini识别的模式组合                        │
│   │       └─ 结合Gemini概率调整置信度                                │
│   ├─ 评分排序 (rank_and_pick - 增强版)                               │
│   │   ├─ 规则评分 (RR + trend + pattern_bonus)                      │
│   │   ├─ Gemini概率加权 (新增)                                       │
│   │   │   └─ 如果Gemini识别了概率，作为额外权重                       │
│   │   ├─ 价格行为预测加权 (新增)                                     │
│   │   │   └─ 如果价格行为学习模型预测匹配，加分                       │
│   │   └─ 综合评分: combine(rule_score, gemini_prob, price_action)  │
│   └─ 输出Top10信号 → trading_signals表 + Markdown文件                │
...
- **模式库**: `src/data/qingniao_abu.duckdb` (表: `pattern_library`)
  - 当前状态: 1004条记录，但`gemini_annotation_json`字段为空（需要重新分析）
  - 目标: 通过Gemini Vision API重新分析1000张图片，提取丰富的结构化信息
...
### 步骤0: Gemini Vision API 分析1000张图片（必须完成）

**现状**：
- 图片数量: 1000张（`data/abu/images/*.png`）
- 数据库状态: pattern_library表已有1004条记录，但`gemini_annotation_json`字段为空
- 问题: 缺少丰富的模式特征、交易参数、多模式组合信息

**解决方案**：使用Gemini Vision API完整分析所有图片
...
#### 阶段1-2: ML模型相关
- `src/ml_dl/abu_price_action_learner.py` - 价格行为特征学习器（最高优先级）
- `src/ml_dl/abu_pattern_match_predictor.py` - 模式匹配预测器
- `src/ml_dl/abu_pattern_weight_optimizer.py` - 权重优化器
- `src/ml_dl/abu_candidate_scorer.py` - ML评分器

#### 阶段1: 数据准备相关
- `scripts/abu/abu_record_match_history.py` - 记录匹配历史
- `scripts/abu/abu_evaluate_signals.py` - 评估ABU信号
- `scripts/abu/abu_backtest_pattern_matches.py` - 回测历史匹配
- `scripts/abu/abu_init_ml_tables.py` - 初始化ML相关表
...
## 文档 2: stage0_gemini_analysis_status

**路径**: `docs\stage_reports\stage0_gemini_analysis_status.md`
...
## 文档 3: stage0_optimization_complete

**路径**: `docs\stage_reports\stage0_optimization_complete.md`
...
**路径**: `docs\design\requirements\技术偏好与优化要求.md`
...
## 文档 5: Gemini_Vision_模块使用说明

**路径**: `docs\design\features\Gemini_Vision_模块使用说明.md`
