...
try:
    ...
    from get_realtime_stop_loss import get_current_price as get_ob_current_price

    ORDERBOOK_AVAILABLE = True
except ImportError as e:
    ORDERBOOK_AVAILABLE = False
    print(
        f"警告: 无法导入实时订单簿止损功能 ({e})，将使用技术指标止损", file=sys.stderr
    )
...
def check_multiple_push(klines, level, tolerance_pct=0.005):
    ...
    for k in klines[-20:]:
        if (
            abs(k["high"] - level) / level < tolerance_pct
            or abs(k["low"] - level) / tolerance_pct < tolerance_pct
        ):
            touches += 1
...
def validate_signal(signal, current_price, tolerance_pct=0.05):
    ...
    if signal_type == "long":
        ...
        if entry > max_entry:
            return (
                False,
                f"做多入场价${entry:,.0f}高于当前价格${current_price:,.0f}超过{tolerance_pct * 100}%，不合理",
            )
        ...
        if entry < current_price:
            return (
                False,
                f"做空入场价${entry:,.0f}低于当前价格${current_price:,.0f}，不合理（做空应等待反弹到阻力位，入场价必须≥当前价格）",
            )
        ...
        if entry == current_price:
            return (
                False,
                f"做空入场价${entry:,.0f}等于当前价格${current_price:,.0f}，建议等待反弹到当前价格上方至少0.1%再入场",
            )
...
def add_signal_analysis_to_plan(plan, best_signal, klines, timeframe_name):
    ...
    try:
        ...
        plan.append(
            f"**盈亏比**: {rr_analysis['avg_rr_ratio']:.2f}:1 ({quality_map.get(rr_analysis['quality'], '未知')})"
        )
        plan.append(
            f"   - 风险: ${rr_analysis['risk']:,.0f} ({abs(best_signal['entry'] - stop_loss_price) / best_signal['entry'] * 100:.2f}%)"
        )
        plan.append(
            f"   - 止盈1回报: ${rr_analysis['reward_1']:,.0f} ({rr_analysis['rr_ratio_1']:.2f}:1)"
        )
        plan.append(
            f"   - 止盈2回报: ${rr_analysis['reward_2']:,.0f} ({rr_analysis['rr_ratio_2']:.2f}:1)"
        )
    except Exception as e:
    ...
    if klines:
        try:
            ...
            if "get_enhanced_volatility_analysis" in globals():
                try:
                    enhanced_vol = get_enhanced_volatility_analysis(
                        klines, current_price, timeframe_name
                    )
                    if enhanced_vol.get("recommended_atr_percent"):
                        atr_percent = enhanced_vol["recommended_atr_percent"]
                        volatility_assessment = enhanced_vol.get(
                            "assessment"
                        ) or assess_volatility_level(atr_percent)

                        # 显示多种波动率指标
                        volatility_info = [
                            f"**市场波动率**: {volatility_assessment['description']}"
                        ]
                        if enhanced_vol.get("atr_traditional_percent"):
                            volatility_info.append(
                                f"传统ATR: {enhanced_vol['atr_traditional_percent']:.2f}%"
                            )
                        if enhanced_vol.get("atr_dynamic_percent"):
                            volatility_info.append(
                                f"动态ATR: {enhanced_vol['atr_dynamic_percent']:.2f}%"
                            )
                        if enhanced_vol.get("adaptive_volatility"):
                            adaptive = enhanced_vol["adaptive_volatility"]
                            volatility_info.append(
                                f"波动率趋势: {adaptive['description']} (短期/长期比值: {adaptive['ratio']:.2f})"
                            )

                        plan.append(" | ".join(volatility_info))
                        plan.append(
                            f"   推荐ATR: {atr_percent:.2f}% (用于短线止损止盈调整)"
                        )
                        ...
                        if volatility_assessment["level"] in ["high", "very_high"]:
                            ...
                            current_stop_distance = (
                                abs(best_signal["entry"] - stop_loss_price)
                                / best_signal["entry"]
                                * 100
                            )
                            suggested_stop_distance = (
                                current_stop_distance
                                * volatility_assessment["stop_loss_multiplier"]
                            )
                            plan.append(
                                f"   ⚠️ 建议止损距离: {suggested_stop_distance:.2f}% (当前: {current_stop_distance:.2f}%)"
                            )
                        ...
                        if enhanced_vol.get("options_iv"):
                            ...
                            plan.append(
                                f"   📊 期权隐含波动率: {iv_data.get('iv_30d', 'N/A')}% (30天)"
                            )
                    else:
                        # 回退到传统ATR
                        ...
                        plan.append(
                            f"**市场波动率**: {volatility_assessment['description']} (ATR: {atr_percent:.2f}%)"
                        )
                        plan.append(f"   {volatility_assessment['recommendation']}")
                except Exception as e:
                    ...
                    order_blocks.append(
                        {
                            ...
                            "strength": "strong"
                            if current_close > prev_high * 1.05
                            else "medium",
                        }
                    )
                    ...
                    order_blocks.append(
                        {
                            ...
                            "candle_count": len(ob_candles),
                            "strength": "strong"
                            if current_close < prev_low * 0.95
                            else "medium",
                        }
                    )
    ...
    if order_blocks:
        ...
        for ob in order_blocks:
            ...
            for existing_ob in unique_obs:
                ...
                if (
                    ob["type"] == existing_ob["type"]
                    and abs(ob["low"] - existing_ob["low"]) / existing_ob["low"] < 0.05
                ):
                    is_duplicate = True
                    break
            if not is_duplicate:
...
def analyze_timeframe(klines, timeframe_name, current_price):
    ...
    if ENHANCED_ANALYSIS_AVAILABLE:
        try:
            ...
            # 用量能增强支撑阻力识别
            enhanced_sr = enhance_support_resistance_with_volume(
                sr, volume_profile, current_price
            )

            # 识别区间（流动性密集区域）
            consolidation_ranges = identify_consolidation_ranges(
                klines[-100:], lookback=50, min_touches=3
            )
        except Exception as e:
    ...
    if ema_144 and ema_169 and len(klines) >= 20:
        ...
        if current_price > ema_169:
            ...
            for i in range(-5, 0):
                if i < -len(klines):
                    continue
                if klines[i]["high"] > ema_169 and (
                    i == -1 or klines[i - 1]["high"] <= ema_169
                ):
        ...
        if current_price > ema_169:
            ...
            if vegas_breakthrough:
                ...
                if is_big_bullish_candle:
                    take_profit_1 = (
                        current_price + (current_price - ema_169) * 0.5
                    )  # 保守目标
                    take_profit_2 = (
                        current_price + (current_price - ema_169) * 1.5
                    )  # 激进目标
                else:
...
def generate_trading_plan():
    ...
    kline_ts = klines_15m[-1].get("timestamp", 0)
    kline_age = now_ts - kline_ts

    # K线数据不应超过30分钟（2个15分钟周期）
    if kline_age > 1800:
        print(f"警告: K线数据过旧 ({kline_age}秒), 重新获取...", file=sys.stderr)
        klines_15m = get_btc_kline_gateio("15m", 200) or get_btc_kline_bitget(
            "15m", 200
        )
        if not klines_15m:
            print("无法获取新鲜K线数据", file=sys.stderr)
            return

    # 价格偏差超过3%时使用K线收盘价（降低阈值以捕获更多异常）
    kline_close = klines_15m[-1]["close"]
    price_diff_pct = (
        abs(kline_close - current_price) / current_price if current_price > 0 else 1
    )
    if price_diff_pct > 0.03:
        print(
            f"警告: 价格偏差{price_diff_pct * 100:.1f}%, 使用K线收盘价 {kline_close}",
            file=sys.stderr,
        )
    ...
    analysis_1h = (
        analyze_timeframe(klines_1h, "1小时", current_price) if klines_1h else None
    )
    ...
    if PATTERN_DETECTOR_AVAILABLE:
        try:
            ...
            if (
                pattern_result_5m["reversal_patterns"]
                or pattern_result_5m["continuation_patterns"]
            ):
                # 将形态转换为交易信号
                for pattern in (
                    pattern_result_5m["reversal_patterns"]
                    + pattern_result_5m["continuation_patterns"]
                ):
                    if (
                        pattern.get("entry")
                        and pattern.get("stop_loss")
                        and pattern.get("take_profit")
                    ):
                        signal_type = (
                            "short" if "bearish" in pattern["type"] else "long"
                        )
                        ...
                        if entry and current_price:
                            ...
                            if signal_type == "long" and entry > current_price * 1.02:
                                ...
                                if entry <= current_price:
                                    ...
                                    if (
                                        pattern.get("resistance")
                                        and pattern["resistance"] > current_price
                                    ):
                                        entry = max(
                                            pattern["resistance"] * 1.002,
                                            current_price * 1.002,
                                        )  # 阻力位上方0.2%，但至少高于当前价格0.2%
                                        entry_reason = f"入场价已调整到{entry:.0f}（等待反弹到阻力位{pattern['resistance']:.0f}上方，至少高于当前价格0.2%）"
                                    elif (
                                        pattern.get("neckline")
                                        and pattern["neckline"] > current_price
                                    ):
                                        # 三重顶等形态可能有颈线作为阻力位
                                        entry = max(
                                            pattern["neckline"] * 1.002,
                                            current_price * 1.002,
                                        )
                        ...
                        # 验证信号合法性
                        is_valid, error_msg = validate_signal(
                        ...
                        if is_valid:
                            ...
                            print(
                                f"5分钟图表形态信号验证失败 ({pattern['name']}): {error_msg}",
                                file=sys.stderr,
                            )
            ...
            if (
                pattern_result_15m["reversal_patterns"]
                or pattern_result_15m["continuation_patterns"]
            ):
                for pattern in (
                    pattern_result_15m["reversal_patterns"]
                    + pattern_result_15m["continuation_patterns"]
                ):
                    if (
                        pattern.get("entry")
                        and pattern.get("stop_loss")
                        ...
                        if is_valid:
                            ...
                            print(
                                f"15分钟图表形态信号验证失败 ({pattern['name']}): {error_msg}",
                                file=sys.stderr,
                            )
        except Exception as e:
    ...
    if DOWNTREND_DETECTOR_AVAILABLE:
        try:
            ...
            # 5分钟时间框架下降趋势识别
            downtrend_signals_5m = downtrend_detector.detect_all_downtrend_signals(
                klines_5m
            )

            # 15分钟时间框架下降趋势识别
            downtrend_signals_15m = downtrend_detector.detect_all_downtrend_signals(
                klines_15m
            )
        except Exception as e:
    ...
    if WASHOUT_DETECTOR_AVAILABLE:
        try:
            ...
            # 5分钟时间框架主力洗盘场景识别
            washout_scenarios_5m = washout_detector.detect_all_washout_scenarios(
                klines_5m
            )

            # 15分钟时间框架主力洗盘场景识别
            washout_scenarios_15m = washout_detector.detect_all_washout_scenarios(
                klines_15m
            )
        except Exception as e:
    ...
    if TAKEOFF_PATTERNS_AVAILABLE:
        try:
            ...
            # 5分钟时间框架K线起飞形态识别
            takeoff_patterns_5m = takeoff_detector.detect_all_takeoff_patterns(
                klines_5m
            )

            # 15分钟时间框架K线起飞形态识别
            takeoff_patterns_15m = takeoff_detector.detect_all_takeoff_patterns(
                klines_15m
            )

            # 1小时时间框架K线起飞形态识别（形态更容易在长时间框架识别）
            if klines_1h:
                takeoff_patterns_1h = takeoff_detector.detect_all_takeoff_patterns(
                    klines_1h
                )
        except Exception as e:
    ...
    if TOP_TRADER_PATTERNS_AVAILABLE:
        try:
            ...
            # 15分钟时间框架顶级交易员形态识别
            top_trader_patterns_15m = top_trader_detector.detect_all_patterns(
                klines_15m
            )

            # 1小时时间框架顶级交易员形态识别（形态更容易在长时间框架识别）
            if klines_1h:
                top_trader_patterns_1h = top_trader_detector.detect_all_patterns(
                    klines_1h
                )
        except Exception as e:
    ...
    if BULLISH_PATTERNS_AVAILABLE:
        try:
            ...
            # 5分钟时间框架看涨形态识别
            bullish_patterns_5m = bullish_detector.detect_all_bullish_patterns(
                klines_5m
            )

            # 15分钟时间框架看涨形态识别
            bullish_patterns_15m = bullish_detector.detect_all_bullish_patterns(
                klines_15m
            )

            # 1小时时间框架看涨形态识别（形态更容易在长时间框架识别）
            if klines_1h:
                bullish_patterns_1h = bullish_detector.detect_all_bullish_patterns(
                    klines_1h
                )
        except Exception as e:
    ...
    if bullish_patterns_5m and bullish_patterns_5m.get("patterns"):
        for pattern in bullish_patterns_5m["patterns"]:
            ...
            take_profit_1 = (
                pattern["entry"] + risk * 2 if risk > 0 else pattern["entry"] * 1.02
            )
            take_profit_2 = (
                pattern["entry"] + risk * 3 if risk > 0 else pattern["entry"] * 1.04
            )
            signal = {
                "type": "long",
    ...
    for signal, timeframe in all_signals_to_process:
        ...
        if is_valid:
            ...
            print(
                f"信号验证失败 ({timeframe}, {signal.get('entry_model', '未知')}): {error_msg}",
                file=sys.stderr,
            )
    ...
    for signal, timeframe in valid_signals_to_process:
        is_valid, error_msg = validate_signal(
            signal, current_price, tolerance_pct=0.001
        )  # 更严格的验证（0.1%容差）
        if is_valid:
            final_valid_signals.append((signal, timeframe))
        else:
            print(
                f"⚠️ 最终验证失败，信号已过滤 ({timeframe}, {signal.get('entry_model', '未知')}): {error_msg}",
                file=sys.stderr,
            )
    ...
    if valid_signals_to_process:
        print(
            f"正在为 {len(valid_signals_to_process)} 个有效信号获取实时订单簿止损...",
            file=sys.stderr,
        )
        for signal, timeframe in valid_signals_to_process:
    ...
    if analysis_5m:
        if analysis_5m["is_garbage_time"]:
            ...
            plan.append(
                f"   区间: ${analysis_5m['garbage_range'][0]:,.0f} - ${analysis_5m['garbage_range'][1]:,.0f}"
            )
            plan.append("")
        ...
        if top_trader_patterns_5m and top_trader_patterns_5m.get("all_signals"):
            for signal in top_trader_patterns_5m["all_signals"]:
                if signal.get("entry") and signal.get("stop_loss"):
                    ...
                    take_profit_1 = (
                        signal["entry"]
                        + (risk * 2 if signal["type"] == "long" else -risk * 2)
                        if risk > 0
                        else (
                            signal["entry"] * 1.02
                            if signal["type"] == "long"
                            else signal["entry"] * 0.98
                        )
                    )
                    take_profit_2 = (
                        signal["entry"]
                        + (risk * 3 if signal["type"] == "long" else -risk * 3)
                        if risk > 0
                        else (
                            signal["entry"] * 1.04
                            if signal["type"] == "long"
                            else signal["entry"] * 0.96
                        )
                    )
                    all_signals_5m.append(
                        {
                            "type": signal["type"],
                            "strength": "strong"
                            if signal.get("strength", 0) > 75
                            else "medium",
                            "entry": signal["entry"],
                            "stop_loss": signal["stop_loss"],
                            "take_profit_1": take_profit_1,
                            "take_profit_2": take_profit_2,
                            "entry_model": signal.get(
                                "entry_model",
                                f"顶级交易员形态-{signal.get('name', '未知')}",
                            ),
                            "reason": signal.get(
                                "reason", f"识别到{signal.get('name', '形态')}"
                            ),
                            "pattern_priority": 1,  # 形态识别优先级标记
                        }
                    )
        ...
        if all_signals_5m:
            ...
            if VOLATILITY_ANALYZER_AVAILABLE and klines_5m:
                try:
                    ...
                    if volatility_assessment["level"] in ["high", "very_high"]:
                        current_stop_distance = (
                            abs(best_signal["entry"] - best_signal["stop_loss"])
                            / best_signal["entry"]
                            * 100
                        )
                        suggested_stop_distance = (
            ...
            # 显示所有识别到的形态（如果有多个）
            pattern_signals = [
                s for s in all_signals_5m if s.get("pattern_priority", 0) > 0
            ]
            if len(pattern_signals) > 1:
                plan.append(
                    f"**识别到 {len(pattern_signals)} 个形态信号**，已选择最强信号"
                )
                plan.append("")
            ...
            strength = (
                "强"
                if best_signal["strength"] == "strong"
                else "中"
                if best_signal["strength"] == "medium"
                else "弱"
            )
            ...
            plan.append(
                f"止损: ${best_signal['stop_loss']:,.0f} (技术指标，距离: {tech_distance_pct:.2f}%)"
            )
            ...
            if best_signal.get("orderbook_stop_loss"):
                ...
                plan.append(
                    f"参考止损: ${ob_stop:,.0f} {ob_status} (距离: {ob_distance_pct:.2f}%)"
                )
                if ob_info.get("info", {}).get("reason"):
                    plan.append(f"   理由: {ob_info['info']['reason']}")

            plan.append(
                f"止盈: ${best_signal['take_profit_1']:,.0f} (50%) / ${best_signal['take_profit_2']:,.0f} (50%)"
            )
            ...
            if VOLATILITY_ANALYZER_AVAILABLE:
                try:
                    ...
                    plan.append(
                        f"**预期盈亏比**: {rr_analysis['avg_rr_ratio']:.2f}:1 {emoji} ({quality})"
                    )
                    plan.append(
                        f"   - 风险: ${rr_analysis['risk']:,.0f} ({abs(best_signal['entry'] - stop_loss_price) / best_signal['entry'] * 100:.2f}%)"
                    )
            ...
            if best_signal.get("ml_score") is not None:
                ...
                plan.append(
                    f"**ML预测成功率**: {ml_score:.1%} (预测: {ml_pred}, 置信度: {ml_confidence:.1%})"
                )
            ...
            if ROLLING_POSITION_AVAILABLE and best_signal:
                try:
                    ...
                    rolling_analysis = rolling_manager.analyze_signal_for_rolling(
                        best_signal, current_price
                    )
                    ...
                    if rolling_analysis.get("suitable"):
                        ...
                        if rolling_analysis.get("suggestion"):
                            rolling_lines = rolling_manager.format_rolling_suggestion(
                                rolling_analysis["suggestion"]
                            )
                            plan.extend(rolling_lines)
                        else:
                            plan.append(
                                f"**分析结果**: {rolling_analysis.get('reason', '适合滚仓策略')}"
                            )
                            plan.append(
                                f"**风险回报比**: {rolling_analysis.get('risk_reward_ratio', 0):.2f}:1"
                            )
                            plan.append("")
                            plan.append("**滚仓策略说明**:")
    ...
    if analysis_1h:
        if analysis_1h["is_garbage_time"]:
            plan.append("⚠️ **垃圾时间**: 建议等待突破")
            plan.append(
                f"   区间: ${analysis_1h['garbage_range'][0]:,.0f} - ${analysis_1h['garbage_range'][1]:,.0f}"
            )
            plan.append("")
        ...
        if all_signals_1h:
            ...
            # 显示所有识别到的形态（如果有多个）
            pattern_signals = [
                s for s in all_signals_1h if s.get("pattern_priority", 0) > 0
            ]
            if len(pattern_signals) > 1:
                plan.append(
                    f"**识别到 {len(pattern_signals)} 个形态信号**，已选择最强信号"
    ...
    if analysis_5m:
        ...
        if analysis_5m.get("ote_analysis"):
            ...
            if ote["in_ote_zone"]:
                ...
                plan.append(
                    f"   - OTE区间: ${ote['fib_618']:,.0f} - ${ote['fib_786']:,.0f}"
                )
        ...
        if analysis_5m.get("order_blocks"):
            ...
            for i, ob in enumerate(analysis_5m["order_blocks"][:3], 1):  # 只显示前3个
                ...
                plan.append(
                    f"  - {i}. {ob_type}: ${ob['low']:,.0f} - ${ob['high']:,.0f} (强度: {ob['strength']})"
                )
    ...
    if analysis_15m:
        if analysis_15m["is_garbage_time"]:
            plan.append("⚠️ **垃圾时间**: 建议等待突破")
            plan.append(
                f"   区间: ${analysis_15m['garbage_range'][0]:,.0f} - ${analysis_15m['garbage_range'][1]:,.0f}"
            )
            plan.append("")
        ...
        if all_signals_15m:
            ...
            # 显示所有识别到的形态（如果有多个）
            pattern_signals = [
                s for s in all_signals_15m if s.get("pattern_priority", 0) > 0
            ]
            if len(pattern_signals) > 1:
                plan.append(
                    f"**识别到 {len(pattern_signals)} 个形态信号**，已选择最强信号"
    ...
    # 三、图表形态识别结果（如果可用）
    if PATTERN_DETECTOR_AVAILABLE and (
        chart_pattern_signals_5m or chart_pattern_signals_15m
    ):
        plan.append("---")
        plan.append("")
        ...
        if chart_pattern_signals_5m:
            ...
            for signal in chart_pattern_signals_5m:
                ...
                plan.append(
                    f"  - 止盈: ${signal['take_profit_1']:,.0f} / ${signal['take_profit_2']:,.0f}"
                )
    ...
    if WASHOUT_DETECTOR_AVAILABLE and (washout_scenarios_5m or washout_scenarios_15m):
        ...
        washout_section = (
            ...
            if not (
                ...
                and (chart_pattern_signals_5m or chart_pattern_signals_15m)
            )
            and not (
                DOWNTREND_DETECTOR_AVAILABLE
                and (downtrend_signals_5m or downtrend_signals_15m)
            )
            else "五"
        )
        if (
        ...
        if washout_scenarios_5m and washout_scenarios_5m.get("scenarios"):
            ...
            plan.append("")
            plan.append(
                f"**做多机会强度**: {washout_scenarios_5m['long_opportunity_strength']:.1f}% (识别到 {washout_scenarios_5m['scenario_count']}/6 种场景)"
            )
            plan.append("")

            for scenario in washout_scenarios_5m["scenarios"]:
                plan.append(
                    f"- **{scenario['name']}** (强度: {scenario['strength']:.1f}%)"
                )
                ...
                if (
                    mapped_key
                    and mapped_key in washout_scenarios_5m["scenarios_details"]
                ):
                    entry = washout_scenarios_5m["scenarios_details"][mapped_key].get(
                        "entry"
                    )
                    if entry:
        ...
        if washout_scenarios_15m and washout_scenarios_15m.get("scenarios"):
            ...
            plan.append("")
            plan.append(
                f"**做多机会强度**: {washout_scenarios_15m['long_opportunity_strength']:.1f}% (识别到 {washout_scenarios_15m['scenario_count']}/6 种场景)"
            )
            plan.append("")
            ...
            for scenario in washout_scenarios_15m["scenarios"]:
                ...
                if (
                    mapped_key
                    and mapped_key in washout_scenarios_15m["scenarios_details"]
                ):
                    entry = washout_scenarios_15m["scenarios_details"][mapped_key].get(
                        "entry"
                    )
                    if entry:
                        plan.append(f"  - 建议入场: ${entry:,.0f}")
    ...
    if TAKEOFF_PATTERNS_AVAILABLE and (takeoff_patterns_5m or takeoff_patterns_15m):
        ...
        if PATTERN_DETECTOR_AVAILABLE and (
            ...
            takeoff_section = "五"
        if DOWNTREND_DETECTOR_AVAILABLE and (
            downtrend_signals_5m or downtrend_signals_15m
        ):
            takeoff_section = "五" if takeoff_section == "四" else "六"
        if WASHOUT_DETECTOR_AVAILABLE and (
            washout_scenarios_5m or washout_scenarios_15m
        ):
        ...
        if takeoff_patterns_5m and takeoff_patterns_5m.get("patterns"):
            ...
            plan.append("")
            plan.append(
                f"**起飞强度**: {takeoff_patterns_5m['takeoff_strength']:.1f}% (识别到 {takeoff_patterns_5m['pattern_count']}/9 种形态)"
            )
            plan.append("")

            for pattern in takeoff_patterns_5m["patterns"]:
                plan.append(
                    f"- **{pattern['name']}** (强度: {pattern['strength']:.1f}%)"
                )
                ...
                if mapped_key and mapped_key in takeoff_patterns_5m["patterns_details"]:
                    entry = takeoff_patterns_5m["patterns_details"][mapped_key].get(
                        "entry"
                    )
                    if entry:
                        plan.append(f"  - 建议入场: ${entry:,.0f}")
        ...
        if takeoff_patterns_15m and takeoff_patterns_15m.get("patterns"):
            ...
            plan.append("")
            plan.append(
                f"**起飞强度**: {takeoff_patterns_15m['takeoff_strength']:.1f}% (识别到 {takeoff_patterns_15m['pattern_count']}/9 种形态)"
            )
            plan.append("")
            ...
            for pattern in takeoff_patterns_15m["patterns"]:
                ...
                if (
                    mapped_key
                    and mapped_key in takeoff_patterns_15m["patterns_details"]
                ):
                    entry = takeoff_patterns_15m["patterns_details"][mapped_key].get(
                        "entry"
                    )
                    if entry:
                        plan.append(f"  - 建议入场: ${entry:,.0f}")
    ...
    if BULLISH_PATTERNS_AVAILABLE and (bullish_patterns_5m or bullish_patterns_15m):
        ...
        if bullish_patterns_5m and bullish_patterns_5m.get("patterns"):
            ...
            plan.append("")
            plan.append(
                f"**看涨强度**: {bullish_patterns_5m['bullish_strength']:.1f}% (识别到 {bullish_patterns_5m['pattern_count']}/8 种形态)"
            )
            plan.append("")
            ...
            for pattern in bullish_patterns_5m["patterns"]:
                ...
                if risk > 0:
                    ...
                    plan.append(
                        f"  - 建议止盈: ${take_profit_1:,.0f} (50%) / ${take_profit_2:,.0f} (50%)"
                    )
                plan.append("")
        ...
        if bullish_patterns_15m and bullish_patterns_15m.get("patterns"):
            ...
            plan.append("")
            plan.append(
                f"**看涨强度**: {bullish_patterns_15m['bullish_strength']:.1f}% (识别到 {bullish_patterns_15m['pattern_count']}/8 种形态)"
            )
            plan.append("")
    ...
    if BULLISH_PATTERNS_AVAILABLE and (bullish_patterns_5m or bullish_patterns_15m):
        section_count += 1
    if TOP_TRADER_PATTERNS_AVAILABLE and (
        top_trader_patterns_5m or top_trader_patterns_15m
    ):
        section_count += 1
    ...
    if analysis_5m:
        ...
        if analysis_5m["fvgs"]:
            ...
            for fvg in analysis_5m["fvgs"][-3:]:
                ...
                plan.append(
                    f"  - {fvg_type}FVG: ${fvg['low']:,.0f} - ${fvg['high']:,.0f}"
                )
            plan.append("")
        ...
        if ENHANCED_ANALYSIS_AVAILABLE and analysis_5m.get("volume_profile"):
            ...
            if vp.get("high_volume_zones"):
                plan.append(
                    f"  - 高量区域数量: {len(vp['high_volume_zones'])} 个（真正的支撑阻力位）"
                )
        ...
        if ENHANCED_ANALYSIS_AVAILABLE and analysis_5m.get("consolidation_ranges"):
            ...
            for i, r in enumerate(ranges[:2], 1):  # 只显示前2个
                plan.append(
                    f"  - 区间{i}: ${r['range_low']:,.0f} - ${r['range_high']:,.0f} (强度: {r['strength']}, 触及{r['touches']}次)"
                )
    ...
    plan.append(
        "2. **Vegas通道信号** (中等): 价格在EMA144/169通道附近，通道作为支撑/阻力"
    )
    ...
    plan.append("")
    plan.append(
        "**建议**: 预生成的信号使用技术指标止损，实时交易时可参考订单簿止损进行调整"
    )
    plan.append("")
    ...
    def format_signal_brief(signal, timeframe_name, klines=None):
        ...
        lines.append(
            f"止损: ${signal['stop_loss']:,.0f} (技术指标，{tech_distance_pct:.2f}%)"
        )
        ...
        if signal.get("orderbook_stop_loss"):
            ...
            lines.append(
                f"参考止损: ${ob_stop:,.0f} {ob_status} ({ob_distance_pct:.2f}%)"
            )

        lines.append(
            f"止盈: ${signal['take_profit_1']:,.0f} (50%) / ${signal['take_profit_2']:,.0f} (50%)"
        )
        ...
        if VOLATILITY_ANALYZER_AVAILABLE:
            try:
                ...
                emoji = quality_emoji.get(rr_analysis["quality"], "")
                lines.append(
                    f"盈亏比: {rr_analysis['avg_rr_ratio']:.2f}:1 {emoji} ({quality})"
                )
            except Exception as e:
                print(f"{timeframe_name}盈亏比计算失败: {e}", file=sys.stderr)
        ...
        if klines and VOLATILITY_ANALYZER_AVAILABLE:
            try:
                ...
                if atr_percent:
                    ...
                    lines.append(
                        f"波动率: {emoji} {volatility_assessment['description']} (ATR: {atr_percent:.2f}%)"
                    )

                    # 如果波动率高，给出止损建议
                    if volatility_assessment["level"] in ["high", "very_high"]:
                        multiplier = volatility_assessment.get(
                            "stop_loss_multiplier", 1.0
                        )
                        suggested_distance = distance_pct * multiplier
                        lines.append(
                            f"⚠️ 高波动建议: 止损距离应调整为 {suggested_distance:.2f}% (当前 {distance_pct:.2f}%)"
                        )
            except Exception as e:
    ...
    if analysis_15m:
        ...
        if all_signals_15m_brief:
            ...
            brief_plan.extend(
                format_signal_brief(best_signal_15m, "15分钟", klines_15m)
            )
        else:
            brief_plan.append("无明确信号")
            brief_plan.append("")
