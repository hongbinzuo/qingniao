...
# -*- coding: utf-8 -*-
"""
基于De.交易系统（融入拍卖理论）生成BCH 1小时交易计划
"""
...
def get_bch_kline_data(timeframe='1h', limit=200):
    """获取BCH K线数据"""
    try:
...
def analyze_trend(klines_1h, klines_4h, klines_1d):
    """分析趋势（多时间框架）"""
    trends = {}
    ...
    if klines_1d and len(klines_1d) >= 20:
        closes_1d = [k['close'] for k in klines_1d[-20:]]
        ema_20_1d = calculate_ema(closes_1d, 20)
        current_1d = klines_1d[-1]['close']
        if current_1d > ema_20_1d:
            trends['1d'] = '偏多'
        else:
            trends['1d'] = '偏空'
...
def analyze_auction_phase(klines):
    ...
    # 判断拍卖阶段
    if avg_price_change < 0.002:  # 价格变化小于0.2%
        phase = "价格接受阶段（Price Acceptance）"
        description = "价格在新区域震荡，成交量稳定，市场接受该价格"
    elif avg_price_change > 0.01:  # 价格变化大于1%
...
def generate_trading_plan():
    """生成交易计划"""
    print("=" * 80)
    print("BCH 1小时交易计划生成（基于De.交易系统 + 拍卖理论）")
    print("=" * 80)
    print()
    ...
    klines_1h = get_bch_kline_data('1h', 200)
    klines_4h = get_bch_kline_data('4h', 100)
    klines_1d = get_bch_kline_data('1d', 50)
    ...
    vwap_1h = calculate_vwap(klines_1h[-24:])  # 最近24小时
    
    # 分析趋势
    trends = analyze_trend(klines_1h, klines_4h, klines_1d)
    
    # 识别价值区域
    value_area = identify_value_area(klines_1h)
    
    # 识别交易区间
    trading_range = identify_trading_range(klines_1h, ema144_1h, ema169_1h)
    
    # 分析拍卖阶段
    auction_phase = analyze_auction_phase(klines_1h)
    
    # 生成交易计划
    plan = []
    plan.append("=" * 80)
    plan.append("BCH 1小时交易计划")
    plan.append("=" * 80)
    plan.append("")
    ...
    # 一、多时间框架趋势分析
    plan.append("## 一、多时间框架趋势分析")
    plan.append("")
    if trends.get('1d'):
        plan.append(f"**日线趋势**: {trends['1d']}")
    if trends.get('4h'):
        plan.append(f"**4小时趋势**: {trends['4h']}")
    if trends.get('1h'):
        plan.append(f"**1小时趋势**: {trends['1h']}")
    plan.append("")
    
    # 判断趋势一致性
    if trends.get('1d') and trends.get('4h') and trends.get('1h'):
        if trends['1d'] == trends['4h'] == trends['1h']:
    ...
    if ema144_1h and ema169_1h:
        ...
        plan.append(f"  - 通道宽度: ${abs(ema169_1h - ema144_1h):,.2f}")
    ...
    if vwap_1h:
        plan.append(f"**VWAP（24小时）**: ${vwap_1h:,.2f}")
    ...
    if trading_range:
        ...
        plan.append(f"  - 区间宽度: ${trading_range['width']:,.2f} ({trading_range['width_points']:.0f}点)")
        
        if trading_range['width_points'] < 100:
            plan.append("  - [WARN] 区间宽度不足100点，不适合区间交易")
        elif trading_range['width_points'] > 500:
            plan.append("  - [WARN] 区间宽度超过500点，需要确认是否为单边行情")
        else:
    ...
    # 检查趋势
    if trends.get('1d') == '偏多' and trends.get('1h') == '偏多':
        reasons.append("[OK] 日线和1小时趋势一致偏多")
    elif trends.get('1d') == '偏空' and trends.get('1h') == '偏空':
        reasons.append("[OK] 日线和1小时趋势一致偏空")
    else:
        reasons.append("[WARN] 趋势不一致，需要谨慎")
    
    # 检查区间
    if trading_range and trading_range['width_points'] >= 100 and trading_range['width_points'] <= 500:
        reasons.append("[OK] 区间宽度合理")
    else:
    ...
    plan.append("  - 距离：通常几百点（根据区间大小调整）")
    if trading_range:
        stop_loss_distance = trading_range['width_points'] * 0.1  # 区间宽度的10%
    ...
    # 保存计划
    plan_text = "\n".join(plan)
    output_file = f"BCH_1h_trading_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
