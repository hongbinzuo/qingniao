...
class OthersDeathCrossAnalysis:
    def __init__(self, symbol="OTHERS"):
        self.symbol = symbol
        self.data = None
        self.death_crosses = []
        self.results = []
    ...
    def generate_sample_data(self, years=5):
        """生成模拟数据用于演示"""
        print(f"生成 {self.symbol} 的模拟数据...")
        
        # 生成5年的周线数据
        start_date = datetime.now() - timedelta(days=years*365)
        dates = pd.date_range(start=start_date, periods=years*52, freq='W')
        
        # 模拟价格走势
        np.random.seed(42)  # 固定随机种子确保结果可重现
        initial_price = 100
        returns = np.random.normal(0.001, 0.05, len(dates))  # 每周平均0.1%收益，5%波动率
    ...
    def calculate_ma(self):
        ...
        print(f"计算移动平均线完成，共 {len(self.data)} 周数据")
        return True
    ...
    def find_death_crosses(self):
        ...
        for i in range(1, len(self.data)):
            prev_ma12 = self.data['MA12'].iloc[i-1]
            prev_ma26 = self.data['MA26'].iloc[i-1]
            curr_ma12 = self.data['MA12'].iloc[i]
            curr_ma26 = self.data['MA26'].iloc[i]
            
            # 死叉条件：前一周MA12 > MA26，当前周MA12 < MA26
            if (prev_ma12 > prev_ma26) and (curr_ma12 < curr_ma26):
                death_cross_date = self.data.index[i]
                death_cross_price = self.data['Close'].iloc[i]
    ...
    def plot_analysis(self):
        ...
        print(f"图表已保存为 {self.symbol}_death_cross_analysis.png")
        plt.close()
...
def main():
    """主函数"""
    print("OTHERS 周线死叉分析程序")
    print("=" * 50)
    
    # 创建分析实例
    analyzer = OthersDeathCrossAnalysis(symbol="OTHERS")
    ...
    if success:
        print("\n分析完成！")
        print("生成的文件：")
        print("- OTHERS_death_cross_statistics.csv (统计表格)")
        print("- OTHERS_death_cross_detailed.csv (详细表格)")
        print("- OTHERS_death_cross_analysis.png (分析图表)")
    else:
        print("\n分析失败！")
