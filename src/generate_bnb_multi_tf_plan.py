...
# -*- coding: utf-8 -*-
"""
根据De.交易系统生成BNB 5分钟、15分钟、1小时交易计划
数据来源：Gate.io / Bitget
"""
...
def get_bnb_kline_gateio(timeframe='5m', limit=200):
    """从Gate.io获取BNB K线数据"""
    try:
...
def get_bnb_kline_bitget(timeframe='5m', limit=200):
    """从Bitget获取BNB K线数据（备用）"""
    try:
...
def get_bnb_current_price():
    """获取BNB当前价格"""
    try:
        url = "https://api.gateio.ws/api/v4/spot/tickers"
        params = {'currency_pair': 'BNB_USDT'}
...
def analyze_liquidity_zones(order_book, current_price):
    ...
    for price, amount in bids:
        # 将价格四舍五入（BNB价格通常以0.1为单位）
        rounded_price = round(price * 10) / 10  # 四舍五入到0.1
...
def analyze_timeframe(klines, timeframe_name, current_price):
    ...
    for fvg in fvgs:
        if fvg['type'] == 'bullish' and current_price < fvg['high']:
            ...
            stop_loss = calculate_smart_stop_loss(entry, 'long', klines, fvgs, sr, key_levels, ema_144, ema_169)
            ...
            entry = fvg['high'] * 0.998  # FVG上沿下方0.2%，挂单等待
            stop_loss = calculate_smart_stop_loss(entry, 'short', klines, fvgs, sr, key_levels, ema_144, ema_169)
            # 优化盈亏比：确保至少1:2.5，目标1:3或更高
    ...
    if sr['support']:
        ...
        if touched_support and price_above_support and is_bouncing:
            # 入场价：挂单在支撑位上方（等待回调）
            entry = nearest_support * 1.005  # 支撑位上方0.5%，挂单等待
            stop_loss = calculate_smart_stop_loss(entry, 'long', klines, fvgs, sr, key_levels, ema_144, ema_169)
            # 止损至少放在支撑位下方2-3%，避免太紧
            if stop_loss > nearest_support * 0.97:
    ...
    if sr['resistance']:
        ...
        if touched_resistance and price_below_resistance and is_rejecting:
            ...
            stop_loss = calculate_smart_stop_loss(entry, 'short', klines, fvgs, sr, key_levels, ema_144, ema_169)
            # 止损至少放在阻力位上方2-3%，避免太紧
            if stop_loss < nearest_resistance * 1.03:
        ...
        if current_price > ema_169:
            if is_breakthrough and is_support:
                ...
                if touched_vegas and is_bouncing:
                    ...
                    stop_loss = calculate_smart_stop_loss(entry, 'long', klines, fvgs, sr, key_levels, ema_144, ema_169)
                    # 止损至少放在Vegas下方2-3%
                    if stop_loss > ema_144 * 0.97:
                        stop_loss = ema_144 * 0.97
                    # 优化盈亏比：确保至少1:2.5
                    risk = entry - stop_loss
                    if risk > 0:
                        take_profit_1 = entry + risk * 2.5  # 1:2.5盈亏比
            ...
            if is_breakthrough and not is_support:
                ...
                if touched_vegas and is_rejecting:
                    ...
                    stop_loss = calculate_smart_stop_loss(entry, 'short', klines, fvgs, sr, key_levels, ema_144, ema_169)
                    # 止损至少放在Vegas上方2-3%
                    if stop_loss < ema_169 * 1.03:
                        stop_loss = ema_169 * 1.03
                    # 优化盈亏比：确保至少1:2.5
                    risk = stop_loss - entry
                    if risk > 0:
                        take_profit_1 = entry - risk * 2.5  # 1:2.5盈亏比
...
def generate_trading_plan():
    """生成交易计划"""
    print("正在获取BNB市场数据...", file=sys.stderr)
    
    # 获取数据
    current_price = get_bnb_current_price()
    klines_5m = get_bnb_kline_gateio('5m', 200)
    klines_15m = get_bnb_kline_gateio('15m', 200)
    klines_1h = get_bnb_kline_gateio('1h', 200)
    
    if not klines_5m:
        print("尝试从Bitget获取数据...", file=sys.stderr)
        klines_5m = get_bnb_kline_bitget('5m', 200)
        klines_15m = get_bnb_kline_bitget('15m', 200)
        klines_1h = get_bnb_kline_bitget('1h', 200)
    ...
    # 保存到文件
    filename = f"BNB_signals_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    # 保存到文件
    filename = f"BNB_multi_tf_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(output)
    print(f"\n交易计划已保存到: {filename}", file=sys.stderr)

def query_bnb_1h_naked_kline():
    """查询BNB 1小时实时价格和裸K分析"""
    print("正在获取BNB 1小时实时数据...", file=sys.stderr)
    
    # 获取数据
    current_price = get_bnb_current_price()
    klines_1h = get_bnb_kline_gateio('1h', 50)
    
    if not klines_1h:
        print("尝试从Bitget获取数据...", file=sys.stderr)
        klines_1h = get_bnb_kline_bitget('1h', 50)
        if not current_price:
            current_price = get_bnb_current_price()
    ...
    # 保存到文件
    filename = f"BNB_1h_naked_kline_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    try:
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(output)
