...
# -*- coding: utf-8 -*-
"""
批量处理图片 761-800 - 使用 Gemini Pro 3
"""
...
print(f"✓ 提示词已加载 ({len(PROMPT)} 字符)")

# ==========================================
# 异步 API 调用
# ==========================================
async def call_gemini_api(session: aiohttp.ClientSession, image_path: str, max_retries: int = 3) -> Optional[Dict]:
    """异步调用 Gemini API"""
...
class DatabaseManager:
    ...
    def save_result(self, page_num: int, image_path: str, gemini_data: Dict, vector_data: Dict) -> Optional[int]:
        """保存识别结果和向量到数据库"""
...
# 主处理函数
# ==========================================
async def process_single_image(session: aiohttp.ClientSession, db: DatabaseManager, page_num: int) -> bool:
    """处理单张图片"""
    ...
    # 调用API分析
    gemini_data = await call_gemini_api(session, image_path)
    if not gemini_data:
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        for i, page_num in enumerate(range(761, 801), 1):
            try:
                result = await process_single_image(session, db, page_num)
