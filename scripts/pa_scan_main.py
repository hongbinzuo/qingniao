...
# -*- coding: utf-8 -*-
"""
Qingniao-Abu Main PA Scanner (Gate/Bybit/Bitget, 5m/15m/1h)

Default: 15m timeframe, hard-coded coins + top volume + top gainers/losers from velo

- K-line numeric detectors produce trade_ready candidates
- Brooks constraints filter/score
- Vector-based pattern matching for similarity search
...
from abu.brooks_pattern_constraints import (  # type: ignore
    BrooksPatternConstraints,
    ConstraintResult,
)
from abu.detectors import detect_all_1h, detect_all_15m  # type: ignore
...
from abu.market_context import (  # type: ignore
...
try:
    from abu.unified_vectorizer import UnifiedVectorizer  # type: ignore
    from abu.vector_pattern_matcher import VectorPatternMatcher  # type: ignore

    VECTOR_MATCHING_AVAILABLE = True
except Exception:
    VectorPatternMatcher = None
    UnifiedVectorizer = None
    VECTOR_MATCHING_AVAILABLE = False

EXCL = set(
    ["USDT", "USDC", "DAI", "BUSD", "FDUSD", "TUSD", "PYUSD", "USDE", "GUSD", "EURT"]
)
IGNORE_SYMBOLS = set(
    [
        "AIR",
...
def _ema_deviation_penalty(
    ...
    penalty = max(
        0.0, min(EMA_DEVIATION_PENALTY_MAX, ratio * EMA_DEVIATION_PENALTY_MAX)
    )
...
def _is_excluded_symbol(symbol: str) -> bool:
    ...
    if sym in IGNORE_SYMBOLS:
        return True
    if sym.endswith("PERP") and sym.replace("-", "").replace("_", "").replace(
        "PERP", ""
    ) in ("RIDE", "TRALA"):
        return True
    if sym in EXCL:
    ...
    if sym.endswith(LEVERAGED_SUFFIXES):
        return True
    # Exclude non-ASCII symbols (Chinese, emoji, etc.)
    if not sym.isascii():
        return True
    return False
...
def _fetch_coincap_symbols(limit: int = 200) -> List[str]:
    try:
        resp = requests.get(COINCAP_URL, params={"limit": int(limit)}, timeout=20)
        if resp.status_code != 200:
            return []
...
def _select_top_movers(
    tickers: List[Dict], count: int = MOVERS_COUNT
...
def _fetch_velo_top_gainers(
    count: int = MOVERS_COUNT, gate_symbols: Optional[set] = None
) -> List[str]:
...
def _check_exchange_health(exchange: str, timeout: int = 5) -> bool:
    ...
    try:
        if exchange == "gate":
            ...
            resp = requests.get(
                "https://api.bybit.com/v5/market/kline", params=params, timeout=timeout
            )
            ok = resp.status_code == 200
...
def _fetch_bitget_symbols() -> List[str]:
    ...
    try:
        resp = requests.get(
            "https://api.bitget.com/api/spot/v1/public/products", timeout=20
        )
    except Exception:
        return symbols
    if resp.status_code != 200:
...
def get_top_coins(
    ...
    if rank_by == "marketcap":
        ...
        source_symbols = _fetch_coincap_symbols(max(200, limit * 5))
        source_name = "coincap"
        if not source_symbols:
            source_symbols = FIXED_MARKETCAP
            source_name = "fixed"

        gate_symbols: set = set()
        tickers: List[Dict] = []
        gate_symbols_empty = False
        if gate_only:
        ...
        if include_movers and tickers:
            ...
            for sym in movers_gainers + movers_losers:
                ...
                symbols.append(sym)
        if include_movers:
            velo_gainers = _fetch_velo_top_gainers(
                movers_count, gate_symbols if gate_symbols else None
            )
        ...
        if len(symbols) < limit and gate_only:
            if not tickers:
                tickers = _fetch_gate_tickers()
            if tickers:
                usdt_pairs = [
                    t for t in tickers if t.get("currency_pair", "").endswith("_USDT")
                ]
                usdt_pairs.sort(
                    key=lambda x: float(x.get("quote_volume", 0)), reverse=True
                )
                for pair in usdt_pairs:
                    sym = pair["currency_pair"].replace("_USDT", "").upper()
                    if sym in symbols or _is_excluded_symbol(sym):
                        continue
                    symbols.append(sym)
                    if len(symbols) >= limit:
        ...
        LAST_TOP_META.update(
            {
                "requested": source_symbols[: max(1, limit)],
                "returned": symbols,
                "not_on_gate": not_on_gate,
                "gate_symbols_empty": gate_symbols_empty,
                "gate_filter": gate_only,
                "movers_added": [
                    s for s in movers_gainers + movers_losers if s in symbols
                ],
                ...
                "marketcap_source": source_name,
            }
        )
        if symbols:
            return symbols
        return source_symbols[: min(limit, len(source_symbols))]
...
def get_kline_exchange(
    symbol: str, timeframe: str, limit: int, exchange: str
) -> List[Dict]:
    ...
    try:
        ...
        with CACHE_LOCK:
            ...
            CACHE_STATS["sources"][stats.source] = (
                CACHE_STATS["sources"].get(stats.source, 0) + 1
            )
        return klines
    except Exception:
        with CACHE_LOCK:
...
def _fetch_group_data(
    ...
    for sym in symbols:
        ...
        if (
            (not kl or len(kl) < 50)
            and fallback_exchange
            and exchange != fallback_exchange
        ):
        ...
        k1h = (
            get_kline_exchange(sym, "1h", limit_1h, used)
            if kl and len(kl) >= 50
            else []
        )
        data[sym] = {
...
def maybe_run_pattern_audit(
    ...
    if report:
        ...
        stamp_path.write_text(
            json.dumps(stamp_payload, ensure_ascii=True, indent=2), encoding="utf-8"
        )
    return report


def build_query_features(
    candidate: Dict, features: Dict[str, object], pattern_type: Optional[str] = None
) -> Dict[str, object]:
...
def estimate_probabilities(
    ...
    if empirical:
        try:
            ...
            p_tp1 = (
                weight * float(getattr(empirical, "p_tp1", p_tp1))
                + (1 - weight) * p_tp1
            )
            p_tp2 = (
                weight * float(getattr(empirical, "p_tp2", p_tp2))
                + (1 - weight) * p_tp2
            )
            p_sl = (
                weight * float(getattr(empirical, "p_sl", p_sl)) + (1 - weight) * p_sl
            )
...
def _adjust_stop_with_rules(
    ...
    countertrend = trend_dir in ("bullish", "bearish") and (
        (trend_dir == "bullish" and direction == "short")
        or (trend_dir == "bearish" and direction == "long")
    )
    ...
    stop_by_pct = (
        ...
        if direction == "long"
        else entry * (1 + min_stop_pct)
    )
    struct_stop = _structure_stop(
        klines, direction, STOP_LOOKBACK.get(timeframe, 20), atr
    )
    if direction == "long":
        target_stop = min(
            [s for s in (stop_by_pct, struct_stop) if s > 0], default=stop_by_pct
        )
        if stop <= target_stop:
            return None
    else:
...
def _is_valid_trade(
    cand: Dict, timeframe: str, klines: List[Dict], features: Dict[str, object]
) -> bool:
    entry = float(cand.get("entry") or 0.0)
    stop = float(cand.get("stop_loss") or 0.0)
...
def _relaxation_notes(
    ...
    if rules.get("allow_against_trend") and trend_strength >= strong_threshold:
        if (trend_dir == "bullish" and direction == "short") or (
            trend_dir == "bearish" and direction == "long"
        ):
            notes.append("逆势放宽")
    ...
    avoid_trend = [
        str(x).lower() for x in (rules.get("trend_mode_avoid_patterns") or [])
    ]
...
def score_with_pattern_library(
    pattern_library, query_features: Dict, candidate_types: List[str]
) -> Tuple[
    float,
    Optional[str],
    float,
    ...
    for ptype in types_to_try:
        ...
        try:
            matches = pattern_library.search_patterns(
                query_features=q, sources=PATTERN_SOURCES, top_k=5, min_confidence=0.2
            )
        except Exception:
            continue
        ...
        score = (
            sum((m.get("similarity", 0.0) * m.get("weight", 0.0)) for m in matches)
            / weight_sum
        )
...
def main() -> None:
    ...
    ap.add_argument("--min-vector-match", type=float, default=0.0)
    ap.add_argument(
        "--trend-mode",
        type=str,
        default="allow",
        choices=["allow", "follow"],
    )
    ap.add_argument("--include-movers", type=int, default=1)
    ap.add_argument(
        "--exchange-mode",
        type=str,
        default="split",
    ...
    ap.add_argument(
        ...
        type=str,
        default="off",
        choices=["off", "brooks", "regime36", "both"],
    )
    args = ap.parse_args()
    ...
    print(
        f"[INFO] timeframe={args.timeframe} days={days} (建议: 5m=3-5天, 15m=10-14天, 1h=30-60天)"
    )
    limit = min(1000, KLINES_PER_DAY.get(args.timeframe, 96) * max(days, 1))

    gate_only = args.exchange_mode == "gate"
    include_movers = bool(args.include_movers)
    ...
    if VECTOR_MATCHING_AVAILABLE:
        try:
            db_manager = TraderDBManager("abu")
            ...
            if index_path.exists() and id_map_path.exists():
                vector_matcher = VectorPatternMatcher(
                    str(index_path), str(id_map_path), db_manager
                )
                vectorizer = UnifiedVectorizer()
                logger.info("Vector matching enabled")
            else:
                logger.warning("Vector index not found, vector matching disabled")
        except Exception as e:
            logger.warning(f"Failed to load vector matcher: {e}")
            vector_matcher = None
            vectorizer = None
    ...
    trend_mode = (args.trend_mode or "allow").lower()
    min_score = float(args.min_score or 0.0)
    min_vector_match = float(args.min_vector_match or 0.0)

    exchange_mode = args.exchange_mode.lower()
    fallback_exchange = (
        "gate" if args.exchange_fallback and exchange_mode == "split" else None
    )
    ...
    if exchange_mode == "split":
        ...
        if "bybit" in availability:
            availability["bybit"] = _load_exchange_symbols(
                "bybit", SYMBOL_CACHE_TTL_HOURS
            )
        if "bitget" in availability:
            availability["bitget"] = _load_exchange_symbols(
                "bitget", SYMBOL_CACHE_TTL_HOURS
            )
        groups = split_symbols(
            symbols, tuple(active_exchanges), availability=availability
        )
        ...
        symbol_data = _fetch_group_data(
            exchange_mode, symbols, args.timeframe, limit, 240, None
        )
    ...
    for sym in symbols:
        ...
        if not kl or len(kl) < 50:
            skip_reasons.setdefault(sym, set()).add(
                f"K线不足或缺失({assigned_exchange})"
            )
            continue
        ...
        for cand in candidates:
            if (
                not cand.get("entry")
                or not cand.get("stop_loss")
                or not cand.get("take_profit_1")
            ):
            ...
            if context_filter_mode != "off":
                ...
                if filter_reason:
                    ...
                    continue
            if trend_mode == "follow":
                trend_dir = str(features.get("trend_direction") or "").lower()
                direction = str(cand.get("type") or "").lower()
                countertrend = (trend_dir == "bullish" and direction == "short") or (
                    trend_dir == "bearish" and direction == "long"
                )
                if trend_dir in ("bullish", "bearish") and countertrend:
                    skip_reasons.setdefault(sym, set()).add("逆势过滤")
                    continue
            ...
            if pattern_library is not None:
                (
                    ...
                    best_page,
                    best_source,
                ) = score_with_pattern_library(
                    pattern_library, query_features, pattern_candidates
                )
                if best_match:
                    gemini_match_count += 1
                    gemini_match_set.add(best_match)
                    mismatch_reason = context_mismatch(
                        context_info, best_match, cand.get("type")
                    )
                    if mismatch_reason:
            ...
            if USE_BROOKS_RULES and constraints is not None:
                ...
                allowed_status = [
                    str(x).lower()
                    for x in (rule_cfg.get("allowed_brooks_status") or [])
                ]
                allow_brooks_fail = "fail" in allowed_status

                constraint = constraints.evaluate(
                    features, pattern_key, direction=cand.get("type")
                )
                ...
                if constraint.status == "fail" and allow_brooks_fail:
                    reason = (cand.get("reason") or "").strip()
                    cand["reason"] = (
                        f"{reason} | Brooks约束放宽" if reason else "Brooks约束放宽"
                    )
            ...
            relax_notes = _relaxation_notes(
                rulebook, cand, features, query_features.get("pattern_type")
            )
            if relax_notes:
                reason = (cand.get("reason") or "").strip()
            ...
            if context_label:
                reason = (cand.get("reason") or "").strip()
                cand["reason"] = (
                    f"{reason} | Context={context_label}"
                    if reason
                    else f"Context={context_label}"
                )
            regime_label = context_info.get("regime")
            if regime_label:
                reason = (cand.get("reason") or "").strip()
                cand["reason"] = (
                    f"{reason} | Regime={regime_label}"
                    if reason
                    else f"Regime={regime_label}"
                )
            regime_36_label = context_info.get("regime_36")
            if regime_36_label:
                reason = (cand.get("reason") or "").strip()
                cand["reason"] = (
                    f"{reason} | Regime36={regime_36_label}"
                    if reason
                    else f"Regime36={regime_36_label}"
                )

            entry_model = f"PA/{base_pattern}" if base_pattern else None
            empirical = prob_estimator.estimate(
                sym, args.timeframe, entry_model=entry_model
            )
            prob_tp1, prob_tp2, prob_sl, prob_source, prob_samples = (
            ...
            vector_score = 0.0
            vector_outcomes = None
            if vector_matcher and vectorizer:
                try:
                    # Extract features for vectorization
                    trend_features = {
                        "direction": features.get("trend_direction", "neutral"),
                        "strength": features.get("trend_strength", 0.5),
                    }
                    ema_features = {
                        "relation": features.get("ema_relation", "at"),
                        "slope": features.get("ema_slope", 0.0),
                    }
                    pattern_features = {
                        "engulfing": "Engulfing" in base_pattern
                        if base_pattern
                        else False,
                        "inside_bar": "InsideBar" in base_pattern
                        if base_pattern
                        else False,
                        "pin_bar": "PinBar" in base_pattern if base_pattern else False,
                        "doji": False,
                        "hammer": False,
                        "shooting_star": False,
                    }
                    ...
                    query_vec = vectorizer.vectorize_live_kline(
                        trend_features, ema_features, pattern_features
                    )

                    # Find similar patterns
                    patterns, distances = vector_matcher.find_similar_patterns(
                        query_vec, k=10
                    )
                    vector_outcomes = vector_matcher.aggregate_outcomes(
                        patterns, distances
                    )

                    # Calculate vector score based on confidence and win probability
                    vector_score = (
                        vector_outcomes["confidence"]
                        * vector_outcomes["win_probability"]
                        * 10.0  # Scale to match other scores
                    )
                except Exception as e:
                    logger.warning(f"Vector matching failed for {sym}: {e}")
                    vector_score = 0.0

            ema_penalty, ema_note = _ema_deviation_penalty(
                cand, features, context_info, args.timeframe
            )
            if ema_note:
                reason = (cand.get("reason") or "").strip()
            ...
            if min_vector_match > 0:
                if not vector_outcomes:
                    skip_reasons.setdefault(sym, set()).add("向量匹配缺失")
                    continue
                vector_conf = float(vector_outcomes.get("confidence") or 0.0)
                if vector_conf < min_vector_match:
                    skip_reasons.setdefault(sym, set()).add(
                        f"向量匹配不足({vector_conf:.2f}<{min_vector_match:.2f})"
                    )
                    continue

            if min_score > 0 and final_score < min_score:
                skip_reasons.setdefault(sym, set()).add(
                    f"评分不足({final_score:.2f}<{min_score:.2f})"
                )
                continue
            ...
            if vector_outcomes:
                enriched["_vector_win_prob"] = vector_outcomes.get(
                    "win_probability", 0.5
                )
                enriched["_vector_rr"] = vector_outcomes.get("typical_rr", 1.5)
                enriched["_vector_confidence"] = vector_outcomes.get("confidence", 0.0)
                enriched["_vector_matches"] = vector_outcomes.get("num_matches", 0)
                enriched["_vector_direction"] = vector_outcomes.get(
    ...
    if args.write_db and top:
        ...
        for r in top:
            try:
                ...
                brooks_note = (
                    f"Brooks:{r.get('_brooks_status')}"
                    if r.get("_brooks_status")
                    else ""
                )
                if brooks_note:
                ...
                print(
                    f"[WARN] 写库失败: {r.get('symbol')} {args.timeframe} {exc}",
                    file=sys.stderr,
                )
        db.close()
    ...
    out = (
        output_dir
        / f"ABU_top{args.top}_{args.timeframe}{context_suffix}_{timestamp}.md"
    )
    ...
    if context_filter_mode != "off":
        ...
        lines.append("")
    if trend_mode != "allow":
        lines.append(f"- 趋势模式: {trend_mode}")
    if min_score > 0:
        lines.append(f"- 最低评分: {min_score:.2f}")
    if min_vector_match > 0:
        lines.append(f"- 最低向量匹配: {min_vector_match:.2f}")
    lines.extend(
        [
    ...
    for i, r in enumerate(top, 1):
        ...
        tp1_fmt = _format_price(
            r.get("take_profit_1") if r.get("take_profit_1") is not None else 0.0
        )
        tp2_fmt = _format_price(
            r.get("take_profit_2") if r.get("take_profit_2") is not None else 0.0
        )
        lines.append(
    ...
    if exchange_mode == "split":
        assigned_line = ", ".join(
            f"{ex}={assigned_counts.get(ex, 0)}" for ex in EXCHANGES
        )
    ...
    lines.append(
        "- 概率来源: 优先使用反馈统计（signal_evaluations）；样本不足时与估算值加权"
    )
    rule_flags = []
    rule_cfg = (rulebook.config or {}).get("rules", {})
    ...
    if USE_BROOKS_RULES:
        if "fail" in [
            str(x).lower() for x in (rule_cfg.get("allowed_brooks_status") or [])
        ]:
    ...
    if PATTERN_SOURCES and any(str(s).startswith("gemini") for s in PATTERN_SOURCES):
        lines.append(
            f"- Gemini 模式实际匹配到的数量: {len(gemini_match_set)} (候选匹配次数={gemini_match_count})"
        )
    if pattern_stats:
        lines.append(
            "- Gemini 模式库统计: "
    ...
    if (
        args.rank_by == "marketcap"
        and LAST_TOP_META.get("gate_filter")
        and LAST_TOP_META.get("gate_symbols_empty")
    ):
        lines.append("- Gate 列表获取失败，按固定币种直接扫描")
    if args.rank_by == "marketcap":
        source_name = LAST_TOP_META.get("marketcap_source")
        if source_name:
            lines.append(f"- 市值来源: {source_name}")
