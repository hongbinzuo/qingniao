...
async def generate_trading_plan(timeframe: str = '15m', top_k: int = 20):
    """
    生成交易计划
    ...
    # 使用最新K线价格
    if abs(klines[-1]['close'] - current_price) / current_price > 0.1:
...
async def main():
    ...
    # 生成5分钟和15分钟交易计划
    print("生成5分钟交易计划...")
    plan_5m = await generate_trading_plan('5m', top_k=20)
    print()
    
    print("生成15分钟交易计划...")
    plan_15m = await generate_trading_plan('15m', top_k=20)
    print()
    ...
    # 保存到文件
    output_file = ROOT / 'trading_signals' / f'ABU_v3_trading_plan_{datetime.now().strftime("%Y%m%d_%H%M%S")}.md'
